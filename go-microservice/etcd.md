...
etcd是基于raft实现的，这里防止数据丢失依靠了raft模块的wal日志。raft提交日志后，commitIndex会加一，kvServer服务会维护一个consistentIndex，用来执行跟进commitndex，并且consistentIndex在数据持久化时也会持久化到db中。如果etcd服务器在持久化未完成时宕机造成数据丢失，在服务器重启时就会重新执行


### 持久化(Durability)
etcd在事务提交时，就会将blotdb
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NjgzODEwNSwxMTgxMjE3NDEyLDkyNj
